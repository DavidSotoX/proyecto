$(document).on('change', '#id_bloque', function () {
    bloque_get_pisos($(this).val(), '#id_numero_piso');
});

function bloque_get_pisos(bloque_id, elemento){
    ...
    if (bloque_id != '') {
        ...
        $.ajax({
            type: 'GET',
            url: url_bloque_pisos + bloque_id,
            processData: false,
            contentType: false,
            success: function (data, ajaxOptions, thrownError) {
                for (var i = 0; i < data.data.length; i++) {
                    $(elemento).append('<option value="' + data.data[i][0] + '">' + data.data[i][1] + '</option>');
                }
            },
